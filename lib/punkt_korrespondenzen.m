function Korrespondenzen = punkt_korrespondenzen(I1,I2,Mpt1,Mpt2,varargin)
    ...
    if do_plot
        ...
        hold on
        % plot points
        plot(Korrespondenzen(1,:),Korrespondenzen(2,:),'go');
        % plot image 2
        imshow(uint8(I2));
        % set transparency to 50%
        alpha(0.5);
        % plot corresponding point
        plot(Korrespondenzen(3,:),Korrespondenzen(4,:),'ro')
        % plot the line
        for i=1:size(Korrespondenzen,2)
            pt1 = [Korrespondenzen(1,i), Korrespondenzen(3,i)];
            pt2 = [Korrespondenzen(2,i), Korrespondenzen(4,i)];
            line(pt1,pt2);
        end
    end
